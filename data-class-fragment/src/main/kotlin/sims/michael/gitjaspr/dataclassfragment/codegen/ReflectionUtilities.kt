...
/**
 * Returns the name of a type without its package name. This differs from the "simple name" in that
 * it includes all enclosing classes in the case of nested class declarations.
 *
 * Example: The "simple name" of `foo.bar.EnclosingClass.EnclosedClass` would be `EnclosedClass`
 * while the "name without package" would be `EnclosingClass.EnclosedClass`
 */
