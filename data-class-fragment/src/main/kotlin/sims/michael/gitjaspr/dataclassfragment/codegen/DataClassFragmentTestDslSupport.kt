...
@DslMarker annotation class TestDslMarker
...
class ScalarIterableBuilder<I : Iterable<T>?, T>(private val build: (Iterable<T>, Boolean) -> I) :
    Builder<I> {
...
class IterableBuilder<I : Iterable<T>?, T, B : Builder<T>>(
    ...
    operator fun invoke(fn: B.() -> Unit) =
        delegate.put((delegate.keys.maxOrNull() ?: -1) + 1, createBuilder().apply(fn))
...
class ScalarMapBuilder<M : Map<String, T>?, T>
private constructor(
...
object BuilderFunctions {
    ...
    inline fun <T, R> ignoringIsNull(crossinline fn: (T) -> R): (T, Boolean) -> R = { src, _ ->
        fn(src)
    }
