...
fun generateUuid(length: Int = 8): String {
    ...
    // Version 4 UUID... extract the time-low and node sections, skipping the portions that contain
    // the version and variant
    val (eight, twelve) =
        requireNotNull("(.{8})(?:-.{4}){3}-(.{12})".toRegex().matchEntire(fullUuid)).destructured
    return (eight + twelve).take(length)
}

fun refsHeads(branch: String) =
    if (!branch.startsWith(GitClient.R_HEADS)) "${GitClient.R_HEADS}$branch" else branch
