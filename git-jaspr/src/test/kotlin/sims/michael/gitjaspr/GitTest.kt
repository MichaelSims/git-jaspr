...
class GitTest {
    ...
    fun `test extractGitHubInfoFromUri`(): List<DynamicTest> {
        fun test(name: String, remoteUri: String, expected: GitHubInfo) =
            dynamicTest(name) { assertEquals(expected, extractGitHubInfoFromUri(remoteUri)) }
        return listOf(
            test("SSH", "git@github.com:owner/name.git", GitHubInfo("github.com", "owner", "name")),
            test(
