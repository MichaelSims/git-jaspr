...
class CliTest {
    ...
    fun `github info is correctly inferred from remote name and URI`(): List<DynamicTest> {
        fun test(name: String, remoteUri: String, remoteName: String, expected: GitHubInfo) =
            dynamicTest(name) {
        ...
        return listOf(
            ...
            test(
                "from other",
                "git@other.com:owner/name.git",
                "other",
                GitHubInfo("other.com", "owner", "name"),
            ),
        )
    }
    ...
    fun `test config happy path`() {
        ...
        val expected =
            config(
                ...
                gitHubInfo =
                    GitHubInfo(host = "github.com", owner = "SomeOwner", name = "some-repo-name"),
                logsDirectory = scratchDir.logsDir(),
            )
        val actual =
            getEffectiveConfigFromCli(
    ...
    fun `invoked from subdirectory finds repo`() {
        ...
        val invokeLocation = scratchDir.repoDir().resolve("my/sub/dir").also { it.mkdirs() }
        val actual =
            getEffectiveConfigFromCli(
                scratchDir,
                remoteUri = "git@github.com:SomeOwner/some-repo-name.git",
    ...
    fun `fails on no git dir`() {
        ...
        val invokeLocation = scratchDir.resolve("otherProject/").also { it.mkdirs() }
        val thrownMessage =
    ...
    fun `configuration priority is as expected`() {
        ...
        val actual =
            getEffectiveConfigFromCli(
                ...
                repoDirConfig =
                    mapOf("repo-owner" to "ownerFromRepoDir", "repo-name" to "nameFromRepoDir"),
                extraCliArgs = listOf("--repo-name", "nameFromCli"),
            )
        assertEquals(ComparableConfig(expected), ComparableConfig(actual))
    }
    ...
    fun `trace logs are written`() {
        ...
        assertTrue(
            lastLogFile.readText().contains("[main]"),
            "$lastLogFile doesn't seem to be a log file",
        )
    }
    ...
    fun `fails if remoteBranchPrefix contains a forward slash`() {
        ...
        val thrownMessage =
            assertThrows<IllegalStateException> {
                ...
                .message
        assertNotNull(thrownMessage)
        assertContains(thrownMessage, "The remote branch prefix should not contain a forward slash")
    }
        ...
        val thrownMessage =
            assertThrows<IllegalStateException> {
                    executeCli(
                        ...
                        DEFAULT_REMOTE_NAME,
                        strings =
                            listOf("--remote-named-stack-branch-prefix", "jaspr-named/", "no-op"),
                    )
                }
                .message
        assertNotNull(thrownMessage)
    ...
    private fun config(workingDirectory: File, gitHubInfo: GitHubInfo, logsDirectory: File) =
        Config(
            workingDirectory,
    ...
    private fun createTempDir(): File {
        val dir =
            checkNotNull(Files.createTempDirectory(CliTest::class.java.simpleName).toFile())
                .canonicalFile
...
object ExecuteCli { // Wrapper object so we can have a logger with a sensible name
    ...
    fun executeCli(
        ...
        val command =
            getInvokeCliList(invokeLocation ?: repoDir, javaOptions) + strings + extraCliArgs
...
private fun File.writeConfigFile(config: Map<String, String>) {
    require(config.keys.none { it.startsWith("--") }) { "Keys should not begin with `--`" }
...
private fun getInvokeCliList(
    workingDir: File = findNearestGitDir(),
    javaOptions: List<String> = emptyList(),
): List<String> {
    // Use the same JDK that invoked us if we can determine it. Else fall back to whatever "java" is
    // in our $PATH
    val javaBinary =
        System.getProperty("java.home")
            ?.let { javaHome -> "$javaHome/bin/java" }
            ?.takeIf { javaHomeBinary -> File(javaHomeBinary).exists() } ?: "java"
    return listOf(
        javaBinary,
