...
class GitHubTestHarnessFunctionalTest {
    ...
    @Test
    fun `can create repo with initial commit`() = withTestSetup {
    ...
    @Test
    fun `can open PRs from created commits`() = withTestSetup(useFakeRemote = false) {
        createCommitsFrom(
            testCase {
                repository {
                    commit { title = "A" }
    ...
    @Test
    fun `can update existing PRs`(testInfo: TestInfo) = withTestSetup(useFakeRemote = false) {
        val m = ident {
